...
**Date:** 2026-08-02 · **Author:** Taj (audit run by a 4-agent team: session-log mining, repo sweep, doctrine grep, worktree triage; PR states verified against GitHub)
...
- **19 worktrees on disk, ~2 GB.** 1.8 GB of that is 10 duplicate npm `node_modules` trees in cortex worktrees.
- **The pileup is review debt, not hygiene debt.** Verified against GitHub: 10 of 12 cortex worktrees back open PRs (#1–#13). Only `issue-18-pushsync` was actually dead (merged, clean, 141 MB). meeting-copilot's 5 worktrees have sat unreviewed since Jul 24.
...
A 12-worktree fleet under npm means 12 full `node_modules` copies (our 1.8 GB). pnpm's content-addressed store makes each worktree install near-instant and near-free on disk. Migrate cortex first; it is where fleets run.
...
- Commit or discard the dirty `docs/vision/understand-the-cortex-vision.md` in `fix-vision-overclaim`, then remove that worktree.
- The real backlog is PR review: 10 open cortex PRs and 5 meeting-copilot branches from Jul 24 are what is keeping 18 of 19 worktrees alive.
