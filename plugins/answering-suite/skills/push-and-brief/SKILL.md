---
name: push-and-brief
description: Push the current branch, then brief Taj in three sections — Shipped, Needs-you, Next. Use when he says "push and brief", "push it and tell me what's next", or "ship it, what do you need from me".
---

# push-and-brief

The ritual that ends a change: the branch goes up, and Taj gets a brief he can
...
prints exactly where it is, and carry that list into the brief so Taj sees what
...
need a rewrite. Rewriting history someone else may hold is Taj's call.
...
**Needs-you** — only what Taj alone can supply: a decision between named options,
...
as in-progress, or comes out. Keep the whole brief inside one screen; Taj reads
...
Read `/Users/taj/.claude/skills/open-items/items.json` and compare its `you`
...
file as you found it — `open-items` keeps item ids stable because Taj's check-offs
...
- Slack belongs to `carson-update`, which owns the cofounder post; this brief is
  terminal output for Taj alone.
- A pull request opens only when Taj asked for one this run — `gh pr create` is
