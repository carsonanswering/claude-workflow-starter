---
name: carson-update
description: Post a work-progress update to cofounder Carson in #code-updates, autonomously. Use when Taj says "/carson-update", "update carson", "cofounder update", or asks what to tell Carson. Optional argument: a topic to lead with.
---

# carson-update

Taj is CEO of Cortex Brain; Carson is his cofounder and the only other person
on the team. Carson is technical but is not in Taj's sessions, so the update is
his entire view of what happened. Write it as normal prose a busy engineer reads
once — never compressed, abbreviated, or telegraphic.

Run this skill fully autonomously. Do not ask Taj anything, do not show a draft
...
- `slack_read_channel` on that channel for the most recent *update* — a message
  ...
  `*In progress*`, `*Blocked on Carson*`, `*Decisions needed*`), whoever posted
...
- `git log --since='<window>' --oneline --all` in each of
  `/Users/taj/projs/meeting-copilot`, `/Users/taj/projs/cortex-brief`,
  `/Users/taj/projs/testing-framework`, `/Users/taj/projs/ai-employees`.
...
- Count a commit as evidence only when it changed behavior, tests, or docs a
  person would care about. cortex-brief commits `chore(<tool>): run <date>`
...
by a commit, a test result, or something Taj said this session, either leave it
...
- **Blocked on Carson** — anything waiting on him, each with the specific action.
- **Decisions needed** — open questions where you name the options and Taj's lean.
...
- If every section came out empty, post nothing. Report to Taj that there was
...
Then report back to Taj: the exact text that went out and the channel it went
...
in the window or to this session, and every "blocked on Carson" line names a
concrete action — with the exact sent text and channel reported back to Taj.
