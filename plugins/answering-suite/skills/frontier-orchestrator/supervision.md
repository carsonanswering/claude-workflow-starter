...
*Worked example (cornea run):* the lease helper is `/Users/taj/projs/.claude/skills/solve-issues/tracker.sh`, and `$TOKEN` is the per-session token that skill has you generate once at the start of a run (`TOKEN="s-$(date +%s)-$RANDOM"`) and reuse for every lease command.

```
/Users/taj/projs/.claude/skills/solve-issues/tracker.sh finish <issue> "$TOKEN" "<pr-url>"
/Users/taj/projs/.claude/skills/solve-issues/tracker.sh stale 0     # every lease outstanding right now
```
