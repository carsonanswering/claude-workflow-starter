...
- **Check what branch the shared checkout is parked on, before you write a single dispatch brief**, and name `origin/main` as the explicit worktree start-point in every one of them:
  ...
  ```bash
  git -C /home/schmi/projs/answering branch --show-current    # if this is not `main`, every bare `worktree add -b` inherits it
  ```
...
/home/schmi/projs/.claude/skills/solve-issues/tracker.sh stale 0     # every lease still held, at any age
```
