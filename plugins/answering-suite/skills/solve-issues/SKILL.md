...
Helper: `/Users/kai/Desktop/projs/.claude/skills/solve-issues/tracker.sh`. Every invocation below spells out that absolute path, because step 4 moves you into a worktree that has no `.claude` of its own. All commands assume it is executable and that `gh` is authenticated.
...
```bash
/Users/kai/Desktop/projs/.claude/skills/solve-issues/tracker.sh next
```
...
```bash
/Users/kai/Desktop/projs/.claude/skills/solve-issues/tracker.sh stale 4
```
...
```bash
/Users/kai/Desktop/projs/.claude/skills/solve-issues/tracker.sh claim <issue> "$TOKEN" || echo "skip, take the next one"
```
...
```bash
cd /Users/kai/Desktop/projs/answering
...
```bash
git -C /Users/kai/Desktop/projs/.worktrees/issue-<n> status --short
git -C /Users/kai/Desktop/projs/answering log --oneline origin/main..agent/issue-<n>
```
...
```bash
ls /Users/kai/Desktop/projs/answering/packages/cornea-authz/src/db/migrations/ \
   /Users/kai/Desktop/projs/.worktrees/*/packages/cornea-authz/src/db/migrations/ 2>/dev/null | sort -u
```
...
```bash
/Users/kai/Desktop/projs/.claude/skills/solve-issues/tracker.sh release <issue> "$TOKEN" "blocked on X"
```
