...
set -e
LIVE=/Users/taj/projs/tracker-live
cd "$LIVE"
./build.sh >/dev/null 2>&1

URL=$(python3 -c "import json,sys;print(json.load(open('/Users/taj/.claude/skills/tracker-refresh/state.json')).get('artifactUrl','UNSET'))" 2>/dev/null || echo UNSET)

python3 - "$URL" <<'PY'
import json, sys, pathlib
d = pathlib.Path('/Users/taj/projs/tracker-live')
...
ready = [i for i in work if not openb(i) and 'needs:taj' not in i['l']]
needs = [i for i in work if 'needs:taj' in i['l']]
