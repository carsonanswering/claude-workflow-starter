...
- Tool use, file edits, or repo-wide context → a `claude-oss -p` worker (read `/Users/kai/.claude/skills/oss-session/SKILL.md` — user-invoked, not reachable by skill name) or a Claude subagent; `fw` has no tools and sees only what you paste.
...
1. Pick `oss` for mechanical work, `ds` only when quality matters — deepseek burns paid thinking tokens and is slower to first token (figures in `/Users/kai/.claude/skills/oss-session/SKILL.md`).
