---
name: skill-sync
description: Sync new or updated skills, agents and hooks into the AnsweringRND/skills share repo and push. Use right after creating or materially changing a skill, agent or hook; when Carson says "sync skills" or "push the skills repo"; or when checking whether the answering-brief trio (day-plan, daily-brief, comp-watch) has drifted between its two copies.
---

# skill-sync

`sync.sh` does every copy, guard and push. Run it instead of hand-typing `cp`/`git` — the hand-typed ritual is what this skill retires, and a skill that never reaches `/home/schmi/projs/skills` exists on this laptop only.
...
```bash
bash /home/schmi/.claude/skills/skill-sync/sync.sh <name>... [-m "commit message"]
```
...
```bash
git -C /home/schmi/projs/skills status -sb | head -1
```

Done when that line reads `## main...origin/main` with no `[ahead N]` marker.

3. Report to Carson: the short hash from step 1, the count, and the item names the script listed under it.

Done when every name the script printed appears in your reply and the hash matches step 2's repo.

## Trio check only

`day-plan`, `daily-brief` and `comp-watch` live at both `/home/schmi/.claude/skills/<name>` and `/home/schmi/projs/answering-brief/.claude/skills/<name>` and must stay byte-identical. To answer "have those copies drifted?" without touching git:

```bash
bash /home/schmi/.claude/skills/skill-sync/sync.sh --check-trio
```
...
- **4** — the commit did not reach origin, because origin holds commits this checkout lacks. Show Carson `git -C /home/schmi/projs/skills log --oneline HEAD..origin/main` and ask before rebasing, merging or forcing anything.
